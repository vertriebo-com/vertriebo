...
function UsageBar({ label, icon: Icon, used, max, color = "bg-primary" }) {
    ...
    <div className="space-y-1.5">
      <div className="flex items-center justify-between text-sm">
        ...
        <span className="font-semibold text-foreground">
          {isUnlimited ? <span className="text-green-600">∞ Unbegrenzt</span> : `${used} / ${max}`}
        </span>
      </div>
      {!isUnlimited && (
    ...
    <div className="space-y-5">
      ...
      <div className="bg-card border border-border rounded-xl p-5 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wide mb-1">Aktueller Plan</h3>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 pt-3 border-t border-border">
          <div>
            <p className="text-xs text-muted-foreground mb-0.5">Aktuelle Periode</p>
          ...
          <div>
            <p className="text-xs text-muted-foreground mb-0.5">Nächste Abrechnung</p>
            <p className="text-sm font-semibold">{formatDate(subscription?.current_period_end)}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground mb-0.5">Trial-Ende</p>
            <p className="text-sm font-semibold">{org?.trial_ends_at ? formatDate(org.trial_ends_at) : "–"}</p>
          </div>
        </div>
      ...
      <div className="bg-card border border-border rounded-xl p-5">
        ...
        {usageLog?.estimated_external_cost_cent > 0 && (
          <div className="mt-4 pt-3 border-t border-border flex items-center justify-between text-xs text-muted-foreground">
            <span className="flex items-center gap-1.5"><Zap className="w-3 h-3" /> Geschätzte externe API-Kosten</span>
            <span className="font-semibold">{(usageLog.estimated_external_cost_cent / 100).toFixed(2)} €</span>
          </div>
        )}
      </div>
      ...
      {plan && (
        <div className="bg-muted/50 border border-border rounded-xl p-4">
          <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Plan-Limits</h3>
