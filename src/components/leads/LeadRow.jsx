...
function statusColor(status) {
  if (status === "Rückruf") return "bg-amber-100 text-amber-800 border-amber-200";
  if (status === "Termin") return "bg-purple-100 text-purple-800 border-purple-200";
  if (status === "Angebot") return "bg-indigo-100 text-indigo-800 border-indigo-200";
  if (status === "Gewonnen") return "bg-emerald-100 text-emerald-800 border-emerald-200";
  if (status === "Verloren") return "bg-slate-100 text-slate-700 border-slate-200";
      ...
      <div className="lg:hidden space-y-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <Link
              to={`/leads/${company.id}`}
              className="text-base font-bold text-slate-900 hover:text-blue-600 transition-colors block mb-1.5"
              title={company.name}
            >
              {company.name}
            </Link>
        ...
        <div className="flex items-center gap-2 pt-2">
          {company.telefon && (
            <a href={`tel:${company.telefon}`} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-700 hover:bg-emerald-100 transition-colors text-sm font-semibold">
              <Phone className="w-4 h-4" /> Anrufen
            </a>
          )}
          <Link
            to={`/leads/${company.id}`}
            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-blue-600 text-white hover:bg-blue-700 transition-colors text-sm font-semibold"
          >
            Details
          </Link>
        </div>
      ...
      <div className="hidden lg:flex items-center gap-5">
        ...
        <div className="flex items-center gap-4 flex-1 min-w-0">
          ...
          <div className="min-w-0 flex-1">
            <Link
              to={`/leads/${company.id}`}
              className="text-lg font-bold text-slate-900 hover:text-blue-600 transition-colors block mb-1"
              title={company.name}
            >
              {company.name}
            </Link>
          ...
          <div className="flex items-center gap-2">
            ...
            {company.telefon && (
              <a
                ...
                className="p-2.5 rounded-lg bg-emerald-50 border border-emerald-200 text-emerald-600 hover:bg-emerald-100 transition-all"
                title="Anrufen"
              >
            ...
            {company.email && (
              <a
                ...
                className="p-2.5 rounded-lg bg-blue-50 border border-blue-200 text-blue-600 hover:bg-blue-100 transition-all"
                title="E-Mail"
              >
            ...
            <div className="relative">
              <button
                ...
                className="p-2.5 rounded-lg bg-white border border-[#E2E8F0] text-slate-700 hover:bg-slate-50 transition-all"
                title="Mehr"
              >
