...
export function analyzeLeadTemperature(company, contactLogs, tasks) {
  ...
  if (temperature === "Hot") {
    if (signals.buying.includes("Termin vereinbart") || signals.buying.includes("Angebot gesendet")) {
      ...
      reason = "Aktive Aufgaben heute und erfolgreicher Kontakt – hohe Priorität.";
    } else {
      reason = "Mehrere positive Signale und hoher Engagement-Score.";
    }
  } else if (temperature === "Warm") {
    if (callbackTasks.length > 0) {
      reason = "Rückruf geplant und relevante Kontaktdaten vorhanden – aktiv verfolgen.";
    } else if (hasSuccessfulContact && company.telefon) {
      reason = "Erstkontakt gelungen und vollständige Kontaktdaten – weitermachen.";
    ...
    if (signals.risks.includes("Noch kein Kontakt dokumentiert")) {
      reason = "Noch kein Kontakt. Telefon oder E-Mail nutzen, um Erstkontakt herzustellen.";
      ...
      reason = "Bisher geringe Engagement-Signale – aktivieren oder priorisieren neu.";
    }
  }
  ...
  if (todayTasks.length > 0) {
    nextBestAction = "heute_aufgaben_erledigen";
  } else if (overdueTasks.length > 0) {
    nextBestAction = "ueberfaellige_aufgaben";
  } else if (!company.telefon && company.email) {
    nextBestAction = "email_senden";
  } else if (!company.telefon && !company.email) {
    nextBestAction = "daten_anreichern";
  }
